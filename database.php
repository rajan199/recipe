...
public function connection()
{
 ...
 public function insert_user($id,$uname,$pass,$add,$city,$zip,$mob,$gen,$ship)
   {
    $con=mysql_connect('localhost','root','');
   mysql_select_db('racipe_database',$con);
   $res=mysql_query("insert into user_tbl values('$id','$uname','$pass','$add','$city','$zip','$mob','$gen','$ship','user')");
   ...
   public function check_type($name,$passs)
   {
    ...
    public function view_some_racipe()
    { 
         
    $con=mysql_connect('localhost','root','');
    mysql_select_db('racipe_database',$con); 
    $res=mysql_query("select r.*,u.* from racipe_tbl as r,user_tbl as u where r.email_id=u.email_id and r.status='available' ORDER BY r.racipe_id desc LIMIT 0, 12",$con);
    return $res;
    }
	
   public function racipe_display()
   {
        $con=mysql_connect('localhost','root','');
        mysql_select_db('racipe_database',$con);  
        $res1=mysql_query("select * from racipe_tbl",$con);
        $cnt=mysql_num_rows($res1);
        return $cnt;
   }
   
  public function view_racipes()
    { 
         
    $con=mysql_connect('localhost','root','');
    mysql_select_db('racipe_database',$con); 
    $res=mysql_query("select r.*,u.* from racipe_tbl as r,user_tbl as u where r.email_id=u.email_id ORDER BY r.racipe_id desc ",$con);
    return $res;
    }  
  


    public function sidebar_recipes_count()
    {
   ...
   public function sidebar_recipes_display()
    {

	$con=mysql_connect('localhost','root','');
    mysql_select_db('racipe_database',$con);
	$cnt1=mysql_query('select * from racipe_tbl group by r.racipe_name',$con);
    return $cnt1;

    }
	
	public function racipebyid($id)
   {
    ...
    $con=mysql_connect('localhost','root','');
    mysql_select_db('racipe_database',$con);
    $res=mysql_query("select * from racipe_tbl where racipe_name LIKE '$ch%'");
    return $res;
	...
	public function recipe_join_cart($eid)
	{
		$con=mysql_connect('localhost','root','');
		mysql_select_db('racipe_database',$con);
		$res=mysql_query("select r.*,o.* from racipe_tbl as r,order_tbl as o where r.racipe_id=o.racipe_id and o.email_id='$eid' and o.status='cart'",$con);
		return $res;
	}
	...
	public function getUserName($id)
   {
	...
	public function racipe_by_id($id)
   {
    $con=mysql_connect('localhost','root','');
    mysql_select_db('racipe_database',$con);
   ...
   public function checkcart($eid,$pid)
    {
        $con=mysql_connect('localhost','root','');
    mysql_select_db('racipe_database',$con);
        $res=mysql_query("SELECT * FROM order_tbl WHERE email_id='$eid' and racipe_id='$pid' and status='cart'",$con);
		$cnt=mysql_num_rows($res);
        return $cnt;
    }
	

public function view_racipe($proid)
	{
		$con=mysql_connect('localhost','root','');
		mysql_select_db('racipe_database',$con);
		$res=mysql_query("select racipe_price from racipe_tbl where racipe_id='$proid'");
		return $res;
	}
	...
	public function racipe_join_history($eid)
	{
		$con=mysql_connect('localhost','root','');
		mysql_select_db('racipe_database',$con);
		$res=mysql_query("select r.*,o.* from racipe_tbl as r,order_tbl as o where r.racipe_id=o.racipe_id and o.email_id='$eid' and o.status='buy' order by o.order_id desc ",$con);
		return $res;
	}
 ...
 public function check_fav($eid,$pid)
    {
        ...
        $res=mysql_query("SELECT * FROM favourite_tbl WHERE email_id='$eid' and racipe_id='$pid'",$this->connection());
		$cnt=mysql_num_rows($res);
        return $cnt;
    }
	
public function display_racipe($fid)
    {
    $res=mysql_query("select * from racipe_tbl where racipe_id='$fid'",$this->connection()); 
	return $res;
    }	
	...
	{
		$res=mysql_query("select r.*,f.* from racipe_tbl as r,favourite_tbl as f where r.racipe_id=f.racipe_id and f.email_id='$eid'",$this->connection());
		return $res;
	}
